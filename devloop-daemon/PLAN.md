...
| **Failures with no compile and no redefine signal** | **measured, fixed** | javac answers for the source and `redefineClasses` for the bytes; neither answers for the app *running*. A view whose re-created instance throws reported `Stable` with the stack trace sitting in `app.log` unread. The daemon now follows that log: every apply marks it, waits `vaadin.dev.errorSettleMillis` (400 ms) after a redefine, reports what was logged on the transaction and in `status`, and escalates to a restart only for linkage and bean-creation failures — an app is free to log an error of its own, and a restart on account of one would be a worse answer than the truth. |
...
| **A green apply over a build that does not resolve** | **found, fixed** | When Maven failed but a previous good classpath existed, the daemon kept it - correct, so `status` keeps answering - and `apply` then reported `no changes` with exit 0 over a project that cannot resolve. The failure is now always recorded: `status` reports it, `apply` fails with it, and only a launch is still allowed to proceed on the last classpath that resolved, with a warning. The reported reason is Maven's first real `[ERROR]` lines rather than the tail of its output, which was always the "-> [Help 1]" epilogue and never the cause. |
| **A module added after the daemon started** | **found, fixed** | The aggregation graph was read once per daemon lifetime, so a module that did not exist then was never in the pom stamp: edits to its own pom were invisible, and `status` never listed it. The reactor is re-read on every resolution now, which is exactly when a pom changed. |
