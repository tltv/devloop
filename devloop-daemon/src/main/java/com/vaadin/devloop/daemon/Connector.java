 ...
 * for the app's lifetime, the daemon sends commands down it and reads the reply.
 * That keeps the app-liveness signal and the command channel as one thing, so
 * there is no second socket to get out of step with the first.
 * <p>
...
final class Connector {
    ...
    static Map<String, String> fields(String reply) {
        ...
        // contains spaces), so it is always last and takes the rest of the line.
        int messageAt = line.indexOf("message=");
        if (messageAt >= 0) {
            fields.put("message", line.substring(messageAt + "message=".length())
                    .trim());
    ...
    static void pump(BufferedReader in, Connector connector, Runnable onActivity)
