...
A reactor can hold more than one Vaadin application, and each gets its own daemon. Pass
`--app <dir>` (or set `VAADIN_DEV_APP`) to say which one a command acts on — `./vaadin-dev --app
../admin apply`. When it is not the default, every command names the application it answered
for, so check that line before trusting the answer.
...
target/devloop/app.log, daemon.log          logs, under the application module
```
...
Playwright assertion patterns, environment variables, and what to do when the loop goes wrong.
